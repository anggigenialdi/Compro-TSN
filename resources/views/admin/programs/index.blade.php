...
@section('main-content')
    <div class="container-fluid">
        <!-- Page Heading -->
        <h1 class="h3 mb-2 text-gray-800">Our Programs</h1>
        <!-- DataTales Example -->
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">List our programs</h6>
                <button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#add-modal">
                    Add
                </button>
                @include('admin.programs.modal-add')
            </div>
            <div>
                {!! Toastr::message() !!}
            </div>
            <div class="card-body">
                <table class="table table-bordered table-stripped table-hover" id="dataTable" cellspacing="0">
                    ...
                    <tbody id="pencarian">
                        @foreach ($dataPrograms as $programs)
                            <tr>
                                ...
                                <td style="height:5px;text-align:center;padding:0px;font-size:12px;">
                                    {{ $programs->title }}</td>
                                <td style="height:5px;text-align:center;padding:0px;font-size:12px;">
                                    <img src="{{ asset('../OurPrograms/'. $programs->picture) }}" title="image preview"
                                        class="img-thumbnail" width="45rem" height="45rem">
                                </td>
                                <td style="height:5px;text-align:center;padding:0px;font-size:12px;">
                                    <!-- Edit Modal -->
                                    <button type="button" class="btn btn-warning btn-sm" data-toggle="modal"
                                        data-target="#editModal{{ $programs->id }}" title="Edit">
                                        Edit
    ...
    @include('admin.programs.javascript')
    @include('admin.programs.modal-edit')
@endsection
